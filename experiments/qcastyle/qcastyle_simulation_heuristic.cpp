...
int main()  // NOLINT
{
    ...
    static const std::string folder = fmt::format("{}qcastyle/gates_h/", EXPERIMENTS_PATH);
    ...
    for (const auto mu : mus ) {
        const sidb_simulation_parameters sim_params_new(2, mu, 4.1, 1.8);
        is_operational_params      isop_params_new{
                sim_params_new,
                sidb_simulation_engine::QUICKSIM
        };

        const sidb_simulation_parameters sim_params_old(2, mu, 5.6, 5);
